...
class StimMetadata(QWidget):
    ...
    def initialise_widget(self, run_complete: int):
        if run_complete: 
            self.show()
            self.get_id()
            self.get_filename()
    ...
    def create_components(self):
        self.open_directory_button = QPushButton(self)
        self.open_directory_button.setText("Save to...")
        self.open_directory_button.clicked.connect(self.set_directory)

        self.directory_label = QLabel(self)
        self.directory_label.setText("Selected directory: ")

        self.export_button = QPushButton(self)
        self.export_button.setText("Export metadata")
    ...
    def get_id(self):
        ...
        self.id = self.date + self.stim_manager.fish_number
    
    def set_directory(self):
        self.directory = QFileDialog.getExistingDirectory(self, "Select Directory")
        if self.directory: 
            self.directory_label.setText(f'Selected directory: {self.directory}')
            self.fish_folder = Path(self.directory) / self.id
            if not self.fish_folder.exists():
                self.fish_folder.mkdir(parents=True)
                print(f"Fish folder '{str(self.fish_folder)}' created.")
                # if self.stim_folder_name:
                #     self.stim_folder_path = self.fish_folder / Path(self.stim_folder_name)
                #     self.stim_folder_path.mkdir(parents=True)
            else: 
                print(f"Fish folder '{str(self.fish_folder)}' already exists.")
                # self.stim_folder_path = self.fish_folder / Path(self.stim_folder_name)
                # if not self.stim_folder_path.exists():
                #     self.stim_folder_path.mkdir(parents=True)

    def get_filename(self):
        self.stim_number = str(self.stim_manager.stim_number)
        self.stim_folder_name = 'stim' + self.stim_number 
        self.filename = self.stim_folder_name + '.json'
    ...
    def export_metadata(self):
        metadata_dict = {
            ...
            'pulse_end': list(self.pulse_end), 
            'pulse_duration': list(self.pulse_duration), 
            'fish_line': self.fishline, 
            'condition': self.condition, 
            'age': self.age,
            'dob': self.dob
        }

        metadata_path = Path(self.fish_folder, self.filename)
